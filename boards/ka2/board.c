...
{
    /* Note: When using an external DCDC, don't set the internal DCDC mode. The following call of pcfg_dcdc_switch_to_dcm_mode() function should be commented out. */
...
#if BOARD_RUNNING_CORE == HPM_CORE0
    if (ptr == HPM_ENET0) {
        init_ptp0_clock(); /* 100MHz */
    } else if (ptr == HPM_ENET1) {
        init_ptp1_clock(); /* 100MHz */
    } else {
        return status_invalid_argument;
    }
#else
    if (ptr == HPM_ENET0) {
        init_ptp0_clock_group1(); /* 100MHz */
    } else if (ptr == HPM_ENET1) {
        init_ptp1_clock_group1(); /* 100MHz */
    } else {
        return status_invalid_argument;
    }
#endif
...
#else
    if (ptr == HPM_ENET0) {
        init_eth0_clock_group1();
    } else {
        init_eth1_clock_group1();
    }
#endif
    if (internal) {
        ...
        if (pllctl_init_int_pll_with_freq(HPM_PLLCTL, PLLCTL_PLL_PLL2, 1000000000UL) == status_success) {
            ...
            init_pll2_postdiv1();
            /* set eth clock frequency at 50MHz for enet0/enet1 */
            if (ptr == HPM_ENET0) {
                init_eth0_rmii_source();
            } else {
                init_eth1_rmii_source();
            }
        } else {
            return status_fail;
        }
...
#else
    if (ptr == HPM_GPTMR0) {
        init_gptmr0_clock_group1();
        freq = clock_get_frequency(clock_gptmr0);
    } else if (ptr == HPM_GPTMR1) {
        init_gptmr1_clock_group1();
        freq = clock_get_frequency(clock_gptmr1);
    } else if (ptr == HPM_GPTMR2) {
        init_gptmr2_clock_group1();
        freq = clock_get_frequency(clock_gptmr2);
    } else if (ptr == HPM_GPTMR3) {
        init_gptmr3_clock_group1();
        freq = clock_get_frequency(clock_gptmr3);
    } else if (ptr == HPM_GPTMR4) {
        init_gptmr4_clock_group1();
        freq = clock_get_frequency(clock_gptmr4);
    } else if (ptr == HPM_GPTMR5) {
        init_gptmr5_clock_group1();
        freq = clock_get_frequency(clock_gptmr5);
    } else if (ptr == HPM_GPTMR6) {
        init_gptmr6_clock_group1();
        freq = clock_get_frequency(clock_gptmr6);
    } else if (ptr == HPM_GPTMR7) {
        init_gptmr7_clock_group1();
        freq = clock_get_frequency(clock_gptmr7);
    } else if (ptr == HPM_PTMR) {
        init_ptmr_clock_group1();
        freq = clock_get_frequency(clock_ptmr);
    } else {
        /* Not supported */
    }
...
{
    if (idle_high == true) {
        init_trgm_gptmr5_cap2_invert_pins();
    } else {
        init_gptmr5_channel0_pin_as_capture();
    }
}
